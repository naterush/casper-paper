...
And we can also define ``protocol executions'' as directed paths between states in $\Sigma$ so that an execution from $\sigma \in \Sigma$ to $\sigma' \in \Sigma$ exists if and only if $\sigma \subseteq \sigma'$. We write the existence of such a state transition as $\sigma \to \sigma'$.
