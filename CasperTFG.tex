...
If we can detect \emph{only some} of the circumstances in which the ideal adversary will fail to find an attack, then we can construct a safety oracle which is able to detect safety in some of the states with estimate safety. The cases may be available enough for it for it to be useful in an implementation even though it may not be an ideal safety oracle.
...
I would like to thank Nate Rush and Danny Ryan for their hard work on getting the codebase ready for first release, for their help on this (draft) paper. Thanks to Karl Floersch, who was the first developer to work on the codebase. I'd like to thank Greg Meredith for an introduction to the ``correct-by-construction'' approach to protocol design, as well as for helping formalize a lot of the ideas. Finally, I'd like to thank Vitalik Buterin, with whom I have collaborated on this research from its very early stages over three years ago.
